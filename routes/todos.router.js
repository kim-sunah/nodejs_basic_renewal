...
/**
 * value 데이터는 필수적으로 존재해야 한다.
 * value 데이터는 문자열 타입이어야 한다.
 * value 데이터는 최소 1글자 이상이어야 한다.
 * value 데이터는 최대 50글자 이하여야 한다.
...
router.post('/todos', async (req, res, next) => {
  try {
    ...
    //3. 존재한다면 현재 해야할 일을+1 하고 .order데이터가 존재하지 않다면, 1로 할당한다.
...
//해야할 일 순서 변경,  완료, 내용 변경
router.patch('/todos/:todoId', async (req, res, next) => {
  const { todoId } = req.params;
  const { order, done, value } = req.body;

  //현재 나의 order 가 무어인지 알아야 한다.
